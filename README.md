...
Personal project I started to eventually be able to play my digital piano (that only has an analog Jack output) in silence. I had no idea about analog audio when I had started this project so I learned (and I still am learning) quite a lot. 

Soon turned out to be a cool project I can learn many new things from. I obviously had more fun listening to the audio coming from my phone's minijack output in real-time and hearing it improve/degrade each time I change something in the circuit or the code.

#### Version 3
![mainpic](images/v3_pic1.jpg)

Take a look at the samples directory - There you can find interesting "milestones" of this project.

## Features
The board continously sends analog audio samples over a serial port to a PC and the host scripts can interpret them in two ways:
...
#### Version 3
- Raspberry Pi Pico
- Minijack connector with soldered on wires
- 2.2uF and 100nF (1uF or 2.2uF would be better) polyester film capacitors for audio coupling
- 4x 47kΩ resistors for applying DC biases (to 2.5V and 1.65V)
- 150Ω resistor in series with the ADC pin
- OPA2134PA configured as unity gain buffer (for buffering the audio line)
- 100pF capacitor for decoupling near the ADC pin
...
All GNDs on the schematic refer to the AGND pin on the RPi Pico. The "noise" symbol refers to the analog input straight from the Jack connector:

![schematic](/schematics/schematic_v3.png)

*The Falstad projects of all circuit versions are in /schematics/*

### Version 2 photo
![pic2](/images/v2_pic1.jpg)